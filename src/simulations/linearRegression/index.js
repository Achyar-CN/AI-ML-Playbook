...
export class LinearRegressionSimulation extends BaseSimulation {
  setup() {
    this.history  = [];
    this.epoch    = 0;
    this._3d      = this._is3DReg; // cache: true when 3D regression CSV loaded
    ...
    // 1D: [1, x, x², …, x^deg]   2D: [1, x, z] or [1, x, z, x², z², xz]
    const nW = this._3d ? (deg >= 2 ? 6 : 3) : deg + 1;
    this.weights = Array.from({ length: nW }, (_, i) =>
  ...
  _polyFeatures(x, z) {
    if (this._3d) {
      ...
      const f = [1, x, z ?? 0];
      if (deg >= 2) f.push(x * x, (z ?? 0) * (z ?? 0), x * (z ?? 0));
      return f;
    }
