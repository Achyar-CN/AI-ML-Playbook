...
export class XGBoostRegressionSimulation extends BaseSimulation {
  ...
  predict(x) {
    const lr = this.params.learningRate || 0.3;
    return this._F0 + this.trees.reduce((s, t) => s + lr * xgbPredict(t, x, 0), 0);
  }
  ...
  step() {
    ...
    // Use 'x' only (y=0 placeholder so xgbPredict works with only x feature)
    const recs = this.points.map((pt, i) => ({ pt: { x: pt.x, y: 0 }, g: this._F[i] - pt.y, h: 1 }));
    const tree = xgbBuildNode(recs, 0, this.params.maxDepth || 3, this.params.minLeafSize || 2, lambda, gamma);
    this.trees.push(tree);
    this.points.forEach((pt, i) => { this._F[i] += lr * xgbPredict(tree, pt.x, 0); });
    this.epoch++;
    this.history.push({ epoch: this.epoch, ...this.computeMetrics() });
  }
